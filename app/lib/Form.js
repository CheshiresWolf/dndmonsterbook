...
function Form(arr) {
    ...
    this.errors = [];
    if (arr.controller) {
        this.__autoBind(arr.controller);
    }
    this.__modelPrepare();
    ...
    this.model.on("clearError", function(field) {
        if (this.model.get("has_error_" + field)) {
...
Form.prototype.validate = function(arr) {
    ...
    _.each(o, function(v, k) {
        if (k.indexOf("has_error_") > -1 || k.indexOf("error_") > -1 || k.indexOf("form_has_error") > -1 || k.indexOf("form_error") > -1) {
            this.model.set(k, "");
        }
    }, this);
    ...
    if (_.isFunction(this.customValidator)) {
        this.errors = _.extend(this.errors, this.customValidator(this.model));
    }
    _.each(this.errors, function(err, field) {
        this.setError(field, err.join(", "));
    }, this);
    if (arr && _.isFunction(arr.handler)) {
        arr.handler(_.extend({}, this.errors));
    }
    if (this.errors) {
...
Form.prototype.__modelPrepare = function() {
    var validation = {};
    _.each(this.bindings, function(b) {
        if (b.object.validation) {
            validation[b.modelField] = _.isFunction(b.object.validation) ? b.object.validation() : b.object.validation;
        }
    });
...
Form.prototype.modelToJSON = function() {
    return _.omit(this.model.toJSON(), function(v, k, obj) {
...
Form.prototype.fillModel = function(json, force) {
    ...
    if (force) {
        ...
        this.model.set(_.pick(json, _.keys(this.model.toJSON())));
    }
    this.validate();
};
...
function grepBindings(controller) {
    //MAGIC: JUST USE ALLOY
    var bindables = _.filter(controller.__views, function(v, k) {
        if (_.isFunction(v.bBind)) {
            v.bBind = v.bBind();
        }
        return !!v.bBind;
    });
...
module.exports = {
    ...
    grepBinds: function(controller, model) {
        var bindables = _rec_findBindables(controller.getView());
        _.each(bindables, function(el) {});
    }
};
